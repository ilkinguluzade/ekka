...
(function (global, factory) {
  ...
  function durationsEqual(d0, d1) {
    return (
      d0.years === d1.years && d0.months === d1.months && d0.days === d1.days && d0.milliseconds === d1.milliseconds
    );
  }
  ...
  function subtractInnerElHeight(outerEl, innerEl) {
    ...
    var diff = outerEl.getBoundingClientRect().height - innerEl.getBoundingClientRect().height; // grab the dimensions
    // undo hack
  ...
  /*! *****************************************************************************
    Copyright (c) Microsoft Corporation. All Rights Reserved by Ilkin Guluzade.
  ...
  function isInteractionPropsValid(state, calendar, dateSpanMeta, filterConfig) {
    ...
    var subjectConfigs = compileEventUis(
      subjectDefs,
      interaction.isEvent ? state.eventUiBases : { "": calendar.selectionConfig } // if not a real event, validate as a selection
    );
    if (filterConfig) {
    ...
    for (var subjectInstanceId in subjectInstances) {
      ...
      // constraint
      if (
        !allConstraintsPass(subjectConfig.constraints, subjectRange, otherEventStore, state.businessHours, calendar)
      ) {
        return false;
      }
      // overlap
  ...
  function isDateSelectionPropsValid(state, calendar, dateSpanMeta, filterConfig) {
    ...
    for (var relevantInstanceId in relevantInstances) {
      ...
      if (rangesIntersect(selectionRange, relevantInstance.range)) {
        ...
        if (
          overlapFunc &&
          !overlapFunc(new EventApi(calendar, relevantDefs[relevantInstance.defId], relevantInstance))
        ) {
          return false;
        }
      }
  ...
  function parseSingle(raw, allDayDefault, calendar, leftovers, allowOpenRange) {
    ...
    if (endMarker) {
      ...
      endMarker = calendar.dateEnv.add(
        startMarker,
        allDay ? calendar.defaultAllDayEventDuration : calendar.defaultTimedEventDuration
      );
    }
    return {
      allDay: allDay,
  ...
  var EmitterMixin = /** @class */ (function (_super) {
    ...
    EmitterMixin.prototype.hasHandlers = function (type) {
      return (
        (this._handlers && this._handlers[type] && this._handlers[type].length) ||
  ...
  function recycleProps(oldProps, newProps, equalityFuncs) {
    ...
    for (var key in newProps) {
      if (
        key in oldProps &&
        (oldProps[key] === newProps[key] || (equalityFuncs[key] && equalityFuncs[key](oldProps[key], newProps[key])))
      ) {
  ...
  var DateEnv = /** @class */ (function () {
    ...
    DateEnv.prototype.toDate = function (m, forcedTzo) {
      if (this.timeZone === "local") {
        ...
        return new Date(
          m.valueOf() - this.namedTimeZoneImpl.offsetForArray(dateToUtcArray(m)) * 1000 * 60 // convert minutes -> ms
        );
      }
    };
    return DateEnv;
    ...
    switch (action.type) {
      ...
      case "CHANGE_TIMEZONE":
        return fetchSourcesByIds(
          eventSources,
          action.sourceIds ? arrayToHash(action.sourceIds) : excludeStaticSources(eventSources, calendar),
          dateProfile ? dateProfile.activeRange : null,
          calendar
        );
      case "RECEIVE_EVENTS":
  ...
  function receiveResponse(sourceHash, sourceId, fetchId, fetchRange) {
    ...
    if (
      ...
      return __assign(
        {},
        sourceHash,
        ((_a = {}), (_a[sourceId] = __assign({}, eventSource, { isFetching: false, fetchRange: fetchRange })), _a)
      );
    }
    return sourceHash;
  }
  ...
  var DateProfileGenerator = /** @class */ (function () {
    ...
    DateProfileGenerator.prototype.buildValidRange = function () {
      return this.getRangeOption("validRange", this.calendar.getNow()) || { start: null, end: null }; // completely open-ended
    };
    ...
    CalendarComponent.prototype.computeHeightVars = function () {
      ...
      if (typeof contentHeightInput === "number") {
        ...
        this.viewHeight = Math.round(
          this.contentEl.getBoundingClientRect().width / Math.max(calendar.opt("aspectRatio"), 0.5)
        );
      }
    };
  ...
  function computeTitle(dateProfile, viewOptions) {
    ...
    return this.dateEnv.formatRange(
      range.start,
      range.end,
      createFormatter(viewOptions.titleFormat || computeTitleFormat(dateProfile), viewOptions.titleRangeSeparator),
  ...
  var View = /** @class */ (function (_super) {
    ...
    function View(context, viewSpec, dateProfileGenerator, parentEl) {
      ...
      _this.renderBusinessHoursMem = memoizeRendering(_this.renderBusinessHours, _this.unrenderBusinessHours, [
        _this.renderDatesMem,
      ]);
      _this.renderDateSelectionMem = memoizeRendering(_this.renderDateSelectionWrap, _this.unrenderDateSelectionWrap, [
        _this.renderDatesMem,
      ]);
      ...
      _this.renderEventDragMem = memoizeRendering(_this.renderEventDragWrap, _this.unrenderEventDragWrap, [
        _this.renderDatesMem,
      ]);
      _this.renderEventResizeMem = memoizeRendering(_this.renderEventResizeWrap, _this.unrenderEventResizeWrap, [
        _this.renderDatesMem,
      ]);
    ...
    View.prototype.startNowIndicator = function (dateProfile) {
      ...
      if (this.opt("nowIndicator")) {
        ...
        if (unit) {
          ...
          delay =
            dateEnv.add(dateEnv.startOf(this.initialNowDate, unit), createDuration(1, unit)).valueOf() -
  ...
  var FgEventRenderer = /** @class */ (function () {
    ...
    FgEventRenderer.prototype.getSegClasses = function (seg, isDraggable, isResizable, mirrorInfo) {
      var classes = ["fc-event", seg.isStart ? "fc-start" : "fc-not-start", seg.isEnd ? "fc-end" : "fc-not-end"].concat(
  ...
  var DayHeader = /** @class */ (function (_super) {
    ...
    DayHeader.prototype.render = function (props) {
      ...
      var colHeadFormat = createFormatter(
        this.opt("columnHeaderFormat") || computeFallbackHeaderFormat(datesRepDistinctDays, dates.length)
      );
      for (var _i = 0, dates_1 = dates; _i < dates_1.length; _i++) {
        var date = dates_1[_i];
        parts.push(
          renderDateCell(date, props.dateProfile, datesRepDistinctDays, dates.length, colHeadFormat, this.context)
        );
      }
      if (this.isRtl) {
        parts.reverse();
  ...
  var Slicer = /** @class */ (function () {
    ...
    Slicer.prototype.sliceProps = function (props, dateProfile, nextDayThreshold, component) {
      ...
      var eventSegs = this.sliceEventStore.apply(
        this,
        [props.eventStore, eventUiBases, dateProfile, nextDayThreshold, component].concat(extraArgs)
      );
      return {
        dateSelectionSegs: this.sliceDateSelection.apply(
          this,
          [props.dateSelection, eventUiBases, component].concat(extraArgs)
        ),
        businessHourSegs: this.sliceBusinessHours.apply(
          this,
          [props.businessHours, dateProfile, nextDayThreshold, component].concat(extraArgs)
        ),
        fgEventSegs: eventSegs.fg,
        bgEventSegs: eventSegs.bg,
        eventDrag: this.sliceEventDrag.apply(
          this,
          [props.eventDrag, eventUiBases, dateProfile, nextDayThreshold, component].concat(extraArgs)
        ),
        eventResize: this.sliceEventResize.apply(
          this,
          [props.eventResize, eventUiBases, dateProfile, nextDayThreshold, component].concat(extraArgs)
        ),
        eventSelection: props.eventSelection,
    ...
    Slicer.prototype.sliceNowDate = function (
      ...
      return this._sliceDateSpan.apply(
        this,
        [{ range: { start: date, end: addMs(date, 1) }, allDay: false }, {}, component].concat(extraArgs)
      );
    };
